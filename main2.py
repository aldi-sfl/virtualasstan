...
def click_virtual():
    ...
    def respond(voice_data):
        ...
        if there_exists(["nama kamu harus"]):
            ...
            engine_speak("baiklah, saya akan mengingat nama saya "+asis_name)
        ...
        if there_exists(["apa kabarmu","bagaimana kabarmu","apa yang sedang kamu lakukan"]):
            engine_speak("saya baik baik saja, bagaimana dengan mu? terimakasih sudah menanyakan")
        ...
        if there_exists(["waktu","jam","jam berapa"]):
            ...
            if time[0]=="00":
                ...
                hours = time[0]
            minutes = time[1]
            time = "sekarang hari " + day + " jam " + hours + " lebih " + minutes + " menit"
            engine_speak(time)
        ...
        # for exit       
        if there_exists(["keluar","sampai jumpa","udahan","udah","sudah","terima kasih"]):
...
class Widget: #GUI OF VIRTUAL ASSISTAND 
    def __init__(self):
        root = Tk()
        root.title('Lumine Virtual Assistant')
        root.geometry('1032x532')

        try:
        # Download the image and save it to the "images" directory
            img = urllib.request.urlretrieve("https://static.zerochan.net/Lumine.full.3126229.jpg", "images/image.jpg")
        except FileExistsError:
        # Do nothing if the file already exists
            pass
        
        img = ImageTk.PhotoImage(file="images/image.jpg")
        ...
        userFrame = LabelFrame(root, text='Lumine', font=('Railways', 24,
        'bold'), bg='gray')
        userFrame.pack(fill='both', expand='yes')

        scrol = Scrollbar(root)
        chat_log = Text(root, state="disabled")
       
        

        
        chat = Entry(root, bd=1, justify=CENTER)
        chat.pack(fill='x', expand='no')
        chat.configure(relief='raised', bd=2)

        label_chat = Label(root, text="🢁 🢁 enter some message 🢁 🢁 or speak instead 🢃 🢃").pack()
       

        send = Button(root, text='send', font=('railways', 10, 'bold'))
        send.pack(fill='x', expand='no')
        speak = Button(root, text='Speak', font=('railways', 10, 'bold'),
        bg='#ff33cc', fg='white',command=click_virtual)
        speak.pack(fill='x', expand='no')
        speak.bind("<ButtonPress>", click_virtual)
        speak.bind("<ButtonRelease>",)
        
        
        
        close = Button(root, text='Close', font=('railways', 10,
        'bold'), bg='#ff0000', fg='white', command=root.destroy)
        close.place(relx=0.32, rely=0.04, anchor="ne")
        root.mainloop()
